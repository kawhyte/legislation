...
const useBills = (selectedJurisdiction: States | null, selectedTopic: string | null) => {
	const params = useMemo(() => {
		...
		logger.debug('[useBills] Selected jurisdiction:', selectedJurisdiction);
		...
		if (selectedJurisdiction) {
			...
			logger.debug('[useBills] Fetching trending bills - API requires q parameter...');
			...
			logger.debug('[useBills] Trending query params:', baseParams);
		}
	...
	const processedData = useMemo(() => {
		if (!rawData || rawData.length === 0) {
			logger.debug('[useBills] No raw data received from API');
			return [];
		}

		logger.debug(`[useBills] Processing ${rawData.length} bills from API`);

		// NEW: Safe trending filtering only for nationwide view
		let filteredBills = [...rawData]; // Safe copy to avoid mutations

		if (!selectedJurisdiction) {
			logger.debug('[useBills] Applying trending filters for nationwide results...');
			...
			logger.debug(`[useBills] Bills by state: Massachusetts: ${massachusettsBills.length}, Other states: ${otherStateBills.length}`);
			...
			if (otherStateBills.length === 0 && massachusettsBills.length > 0) {
				logger.warn('[useBills] WARNING: API only returned Massachusetts bills. This suggests a query limitation.');
				logger.warn('[useBills] Recommendation: Contact API provider or use different query strategy');
			}
			...
			if (otherStateBills.length > 0) {
				...
				logger.debug(`[useBills] Balanced state distribution: MA: ${limitedMassBills.length}, Others: ${otherStateBills.length}`);
			}
			...
			logger.debug(`[useBills] Bills with recent activity: ${recentActivityBills.length}/${filteredBills.length}`);
		...
		logger.debug(`[useBills] Final result:`, {
